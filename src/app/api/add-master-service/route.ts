...
export async function POST(request: Request) {
  ...
  if (!name || !price || !category || durationMinutes < 15) {
    return NextResponse.json(
      { error: "Name, price, category, and duration are required." },
      { status: 400 }
    );
  }

  const { error } = await supabaseAdmin.from("master_services").insert([
    {
      master_slug: slug,
      name,
      price,
