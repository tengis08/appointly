...
async function getLoggedMasterAccount() {
  ...
  const { data: account, error: accountError } = await supabaseAdmin
    ...
    .select(
      "user_id, master_slug, plan_type, subscription_status, calendar_feed_token"
    )
    .eq("user_id", user.id)
    .maybeSingle();
...
export async function POST(request: Request) {
  ...
  if (!account.calendar_feed_token) {
  ...
  return NextResponse.redirect(
    new URL(`/dashboard/${slug}?calendar=created`, request.url),
    { status: 303 }
  );
}
